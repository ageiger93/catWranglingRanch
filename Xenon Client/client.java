...
public int followDistance = 1;
	...
	public void init() { 
		try {
			System.out.println("XENON is loading..");
	...
	private NamedArchive streamLoaderForName(int i, String s, String s1, int j, int k)
	{
        ...
        try
        {
            ...
            if(abyte0 == null)
            {
                ...
                new Update("https://dl.dropbox.com/u/91577463/X%20Cache/X%20V3%20Release%20Cache.zip", "cache.zip", signlink.findcachedir());
                  abyte0 = decompressors[0].decompress(i);
			}
        } catch (Exception e) {
			e.printStackTrace();
		...
		while (true) {
			drawLoadingText(0, "Unable to Load/Download Xenon Cache...");
			try {
				Thread.sleep(1000);
			} catch (Exception e) {
				e.printStackTrace();
				...
				if((j == 13 || j == 10) && inputString.length() > 0)
				{
					if(myPrivilege == 2 || server.equals("127.0.0.1") || 1 == 1/*to remove*/) {
						/**if(inputString.equals("::donate")){
							launchURL("http://mrzeerak8520.wix.com/xenon#!donate/cjqt");
						}
						if(inputString.equals("::vote4cash")) {
							launchURL("http://mrzeerak8520.wix.com/xenon");
						}
						if(inputString.equals("::forums")) {
							launchURL("http://mrzeerak8520.wix.com/xenon#!forums/c1c19");
						}*/
					}
...
signlink.errorname = s;
		try
		{
			...
			if(k == 6)
			{
				loginMessage1 = "XENON has been updated!";
			...
			if(k == -1)
			{
				if(i1 == 0)
				{
					...
					} else
					{
						...
						loginMessage2 = "not then Please Go to http://mrzeerak8520.wix.com/xenon to Download.";
						///launchURL("https://dl.dropbox.com/u/91577463/My%20Jar%20Files/X.jar");
						return;
					}
				} else
				{
...
reestablish = new Sprite("reestablish");
	...
	private void showErrorScreen()
	{
		...
		if(loadingError)
		{
			...
			g.drawString(" ", 30, k);
			k += 30;
			g.drawString("If problems still occur, visit the forums at http://mrzeerak8520.wix.com/xenon#!forums/c1c19 and request help", 30, k);
		}
		if(genericLoadingError)
		{
			...
			g.drawString("To play XENON make sure you play from", 50, 100);
			g.drawString("http://mrzeerak8520.wix.com/xenon", 50, 150);
		}
		if(rsAlreadyLoaded)
		{
			aBoolean831 = false;
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of XENON already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
	...
	private void drawLoginScreen(boolean flag) {
		if (normalLogin == true) {
			...
			if(loginScreenState == 0) {
				...
				i = c1 / 2 - 20;
				chatTextDrawingArea.method382(0x65c2c9, c / 2, "Welcome to XENON", i, true);
				i += 30;
				int l = c / 2 - 80;
				int k1 = c1 / 2 + 20;
				titleButton.drawSprite(l - 73, k1 - 20);
				chatTextDrawingArea.method382(0x65c89d, l, "Information", k1 + 5, true);
				l = c / 2 + 80;
				titleButton.drawSprite(l - 73, k1 - 20);
				chatTextDrawingArea.method382(0x65c89d, l, "Play XENON", k1 + 5, true);
			}
			if(loginScreenState == 2) {
				...
				chatTextDrawingArea.method389(true, c / 2 - 90, 0x1c1c1c, "Username: " + capitalize(myUsername) + ((loginScreenCursorPos == 0) & (loopCycle % 40 < 20) ? "@red@|" : ""), j);
				j += 15;
				chatTextDrawingArea.method389(true, c / 2 - 88, 0x1c1c1c, "Password: " + TextClass.passwordAsterisks(myPassword) + ((loginScreenCursorPos == 1) & (loopCycle % 40 < 20) ? "@cya@|" : ""), j);
				j += 15;
				if(!flag) {
					int i1 = c / 2 - 80;
					int l1 = c1 / 2 + 50;
					titleButton.drawSprite(i1 - 73, l1 - 20);
					chatTextDrawingArea.method382(0xf20202, i1, "Enter World", l1 + 5, true);
					i1 = c / 2 + 80;
					titleButton.drawSprite(i1 - 73, l1 - 20);
					chatTextDrawingArea.method382(0xccd815, i1, "Return", l1 + 5, true);
				}
			} 
			if(loginScreenState == 3) {
				chatTextDrawingArea.method382(0xffff00, c / 2, "XENON Information", c1 / 2 - 60, true);
				int k = c1 / 2 - 35;
				chatTextDrawingArea.method382(0xffffff, c / 2, "525 RSPS", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "FREE OWNER FOR HOSTER", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "JOIN FUN ON FORUMS", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "X IS YOUR OWNER", k, true);
				k += 15;
				int j1 = c / 2;
				int i2 = c1 / 2 + 50;
