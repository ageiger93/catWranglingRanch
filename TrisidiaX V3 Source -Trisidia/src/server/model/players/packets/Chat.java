...
c.sendMessage("You are muted, Type ::appeal [Reason of appeal] to appeal your mute.");
return;
} else {
c.muteEnd = 0;
}
String[] stuffz = { ",net", ",runescape", ",org", ",com", ",be", ",nl", ",info", "dspk",".info" , ".org", ".tk", ".net", ".com", ".co.uk", ".be", ".nl", ".dk", ".co.cz", ".co", ".us", ".biz", ".eu", ".de", ".cc", ".i n f o", ".o r g", ".t k", ".n e t", ".c o m", ".c o . u k", ".b e", ".n l", ".d k", ".c o . c z", ".c o", ".u s", ".b i z", ".e u", ".d e", ".c c"};
					...
					c.getPA().writeMassChatLog(term);
			if(c.said >= 15000 && c.playerRights <= 90){
			c.sendMessage("You have been muted!");
			Connection.addNameToMuteList(c.playerName);
			}
			for(int i = 0; i < stuffz.length; i++) {
					if(term.contains(stuffz[i]) && c.playerRights <= 90) {
					c.said++;
...
if (!Connection.isMuted(c)){
	...
	c.sendMessage("You are muted, no one can hear you, Type ::appeal [Reason of appeal] to appeal your mute.");
	return;
		}
	}
}