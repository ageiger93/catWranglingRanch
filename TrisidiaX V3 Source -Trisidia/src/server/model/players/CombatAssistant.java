	  ...
	  if (c.playerEquipment[c.playerWeapon] == 15241) {
		...
		if (c.inWG()) {
			...
			if(c.attackTimer <= 0) {
				...
				c.faceUpdate(i);
				c.slayerHelmetEffect = c.playerEquipment[c.playerHat] == 13263 && c.slayerTask == i;
			...
			c.followId2 = i;
				...
				//	}, 800);
					...
					System.out.println("Soulsplit GFX Instance Failed [Error Prevented]");
				}
			}
		}
	}
...
c.specAmount += c.specAmount*0.1;
	...
	**/
	...
	public boolean checkReqs() {
		...
		if(!Server.playerHandler.players[c.playerIndex].inPits() && !Server.playerHandler.players[c.playerIndex].inWild() && !Server.playerHandler.players[c.playerIndex].InDung() && !Server.playerHandler.players[c.playerIndex].inDuelArena() && Server.playerHandler.players[c.playerIndex].safeTimer <= 0) {
		...
		if(Server.playerHandler.players[c.playerIndex].inDuelArena() && c.duelStatus != 5 && !c.usingMagic) {
			...
			if(Server.playerHandler.players[c.playerIndex].InDung()) {
			c.sendMessage("This feature is still in development!");
			c.getCombat().resetPlayerAttack();
			return false;
		}
 ...
 if(weaponName.contains("2h sword") || weaponName.contains("godsword") || weaponName.contains("saradomin sw")) {
	...
	**/
	...
	public int calculateMeleeAttack() {
		...
		} else if (c.slayerHelmetEffect && c.slayerTask != 0) {
...
return;
	...
	public void handleGmaulPlayer() {
		if (c.playerIndex > 0) {
			...
			if (c.goodDistance(c.getX(), c.getY(), Server.npcHandler.npcs[c.npcIndex].getX(), Server.npcHandler.npcs[c.npcIndex].getY(), getRequiredDistance())) {
				if (checkReqs()) {
					if (checkSpecAmount(4153)) {						
