...
public class PlayerSave
{
	...
	**/
	public static int loadGame(Client p, String playerName, String playerPass) {
		...
		try {
			characterfile = new BufferedReader(new FileReader("./Data/1.)TrisidiaX Player/1.)Characters/"+playerName+".txt"));
			File1 = true;
...
p.SETHOMEY = Integer.parseInt(token2); 
    ...
    p.ag6 = Integer.parseInt(token2);
                    ...
                    p.playerTitle = Integer.parseInt(token2);
						...
						p.pkPoints = Integer.parseInt(token2);	
										...
										} else if (token.equals("voteAmount")) {
					p.voteAmount = Integer.parseInt(token2);
					...
					} else if (line.startsWith("playersPked")) {
						p.playersPked = Integer.parseInt(token2);
					} else if (line.startsWith("amountDied")) {
						p.amountDied = Integer.parseInt(token2);
	...
	**/
	public static boolean saveGame(Client p) {
		...
		try {
			characterfile = new BufferedWriter(new FileWriter("./Data/1.)TrisidiaX Player/1.)Characters/"+p.playerName+".txt"));
			...
			characterfile.newLine();
			characterfile.write("amountDied = ", 0, 13);
            characterfile.write(Integer.toString(p.amountDied), 0, Integer.toString(p.amountDied).length());
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("votePoints = ", 0, 12);
            characterfile.write(Integer.toString(p.votePoints), 0, Integer.toString(p.votePoints).length());
			characterfile.newLine();
			characterfile.write("voteAmount = ", 0, 12);
            characterfile.write(Integer.toString(p.voteAmount), 0, Integer.toString(p.voteAmount).length());
			characterfile.newLine();
			characterfile.newLine();
            ...
            characterfile.write(Long.toString(p.pTime), 0, Long.toString(p.pTime).length());
            characterfile.newLine();
