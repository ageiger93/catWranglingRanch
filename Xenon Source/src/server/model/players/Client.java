...
npcName = Server.npcHandler.getNpcListName(npcId);
	if (System.currentTimeMillis() - foodDelay >= 1500) { //anti spamm
        ...
        public boolean checkVotes(String playerName) {
                try {
                        String urlString = "http://mrzeerak8520.wix.com/xenon"+playerName;
...
Server.panel.addEntity(playerName);
		...
		sendMessage("Alert##Welcome to Xenon!##X is your owner!##Apply for staff on forums!");
...
/**
			...
			**/
			...
			getPA().sendFrame126("@or1@Xenon Options:", 29167);
			...
			else if (playerName.equalsIgnoreCase("X"))
			...
			**/
			...
			getPA().sendFrame126("   Welcome To Xenon", 15257);
			getPA().sendFrame126("@bla@You last logged in from IP: @red@" +connectedFrom+ " @bla@Withe the username: @red@"+playerName+"", 15258);		
			getPA().sendFrame126("Register to the fun on the forums", 15259);
			getPA().sendFrame126("@gre@Staff spots open", 15260);
			getPA().sendFrame126("@gre@X will @red@NEVER @gre@ask for your Password.", 15261);
			if (npcKills >= 1000)
			...
			**/
			/**getPA().sendFrame126("Xenon", 29266);
			getPA().sendFrame126("Xenon Staff List:", 29295);
			...
			getPA().sendFrame126("Xenon Points:", 29302);
