/* 
WELCOME TO THE SOURCE I AM USING! THIS SOURCE IS BEING USED FOR XENON!
OWNER: X
   ...
   public void playerCommands(Client c, String playerCommand)
    {
		...
		**/
		...
		if (Connection.isMuted(c)) {
				...
				c.sendMessage("X will Approve or Disapprove your Appeal.");
				} finally {
				report.close();
			}
			} catch (IOException e) {
		...
		c.sendMessage("You must be either muted or Jailed to post an Appeal -X");
		}
		...
		if(c.playerRights == 0 && !playerCommand.startsWith("::")) {
			...
			if (playerCommand.equals("store")) {
				c.getPA().startTeleport(3233, 2898, 0, "modern");
				c.sendMessage("All the shops! Enjoy!");
			}

			if (playerCommand.equals("staffzone") && c.playerRights <= 1) {
				c.getPA().startTeleport(1823, 4835, 1, "modern");
			}

			if (playerCommand.equals("dp") && c.playerName.equalsIgnoreCase(""+ Config.OWNER +"")) {
							c.teleportToX = 2739;
                        				c.teleportToY = 3472;
                        				c.heightLevel = 0;
								c.sendMessage("Mass teleport to: " + c.playerName + "");
								c.sendMessage("Alert##DROP PARTY##X");
			}

			if (playerCommand.equals("ownerzone") && c.playerRights == 3) {
				c.getPA().startTeleport(3094, 3107, 0, "modern");
				c.sendMessage("!!!OWNERS ONLY!!!");
			}
			...
			if(playerCommand.startsWith("withdraw")) {
				...
				if (c.inWild()) {
				c.sendMessage("<shad=16745472>[XENON]</col><shad=65288>You cannot do this in the wilderness");
...
if (playerCommand.equalsIgnoreCase("players")) {
						...
						} else if (p.playerName.equalsIgnoreCase("X")) {
						title = "@cya@XXX@blu@, ";
						} else if (p.playerName.equalsIgnoreCase("")) {
			...
			if(c.inWild()) {
				c.sendMessage("<shad=16745472>[XENON]</col><shad=65288>Er, it's not to smart to do this in the Wilderness.");
				return;
			}
				c.sit = false;
			...
			if (playerCommand.startsWith("funpk")) {
				...
				c2.sendMessage("<shad=16745472>[XENON]</col><shad=65288> " + c.playerName + " Is now at ::funpk!!");
			}
			}
			*/
			/**
			if (playerCommand.startsWith("teampk")) {
				...
				if (Server.playerHandler.players[i] != null) {
				Client c2 = (Client)Server.playerHandler.players[i];
				c2.sendMessage("<shad=16745472>[XENON]</col><shad=65288> " + c.playerName + " Is now at ::teampk!!");
			}
			}
			}
			*/
		...
		Server.clanChat.handleLootShare(c, item, 1);
			...
			if(c.InDung()) {
                        c.sendMessage("<shad=16745472>[XENON]</col><shad=65288>You cannot un-sit in Dungoneering");
                        return;
                        }
                        if(c.inWild()) {
			c.sendMessage("<shad=16745472>[XENON]</col><shad=65288>You cannot do this in the wilderness.");
			...
			if (playerCommand.equalsIgnoreCase("help")) {
				if(c.inWild()) {
		c.sendMessage("<shad=16745472>[XENON]</col><shad=65288> You can not ask for help in the Wild!");
		return;
		}
		RequestHelp.callForHelp(c);
			...
			if(System.currentTimeMillis() < c.muteEnd) {
				c.sendMessage("<shad=16745472>[XENON]</col><shad=65288>You are muted and cannot yell, Type ::appeal [Reason of appeal] to appeal your mute.");
				return;
				} else {
				c.muteEnd = 0;
				}
	...
	{			
				...
				for (int i = 0; i < Config.MAX_PLAYERS; i++) {
					if (Server.playerHandler.players[i] != null) {
						 Client c2 = (Client)Server.playerHandler.players[i];
						c2.sendMessage("<shad=16745472>[XENON]</col><shad=65288> " +c2.playerName+ " Got banned by. " + c.playerName+ ".");
			...
			c.sendMessage("Player Must Be Offline.");
						...
						if(c.inWild()) {
								c.sendMessage("<shad=16745472>[XENON]</col><shad=65288>You can not tele to someone in the wild!");
								return;
								}
						RequestHelp.teleportToPlayer(c);
							...
							if(c.inWild()) {
							c.sendMessage("<shad=16745472>[XENON]</col><shad=65288> get out of the wild to jail-unjail!");
							return;
							}
							if(c.InDung()) {
	...
	**/
	...
	if (playerCommand.startsWith("reloadspawns") && c.playerRights >= 3) {
						...
						for (int j = 0; j < Server.playerHandler.players.length; j++) {
							if (Server.playerHandler.players[j] != null) {
								Client c2 = (Client)Server.playerHandler.players[j];
								 c2.sendMessage("<shad=15695415>[X News]:" + c.playerName + " " + " Has reloaded all Npc Spawns.</col> " + Misc.optimizeText(playerCommand.substring(3)));
							}
						}
		...
		if (playerCommand.startsWith("reloadshops") && c.playerName.equalsIgnoreCase(""+ Config.OWNER +"")) {
								...
								for (int j = 0; j < Server.playerHandler.players.length; j++) {
									if (Server.playerHandler.players[j] != null) {
										Client c2 = (Client)Server.playerHandler.players[j];
												  c2.sendMessage("<shad=15695415>[X News]:" + c.playerName + " " + " Has refilled the shops.</col> " + Misc.optimizeText(playerCommand.substring(3)));
									}
								}
							}
			...
			if (playerCommand.indexOf(" ") > -1 && c.playerRights > 1) {
			   ...
			   if(c.inWild()) {
			c.sendMessage("<shad=16745472>[XENON]</col><shad=65288> You can not spawn item in wildy!");
			return;
			}
				try {
				String[] args = playerCommand.split(" ");
				...
				if(i == newItemID) {
				c.sendMessage("<shad=16745472>[XENON]</col><shad=65288> "+ Config.OWNER +" Has disabled this item from being spawned.");
				return;
				}
				}
			...
			if (playerCommand.startsWith("interface")) {
							...
							if (playerCommand.startsWith("sm") && c.playerName.equalsIgnoreCase(""+ Config.OWNER +"")) {
								for (int j = 0; j < Server.playerHandler.players.length; j++) {
									if (Server.playerHandler.players[j] != null) {
										Client c2 = (Client)Server.playerHandler.players[j];
										c2.sendMessage("<shad=15695415>[X News]</col> " + Misc.optimizeText(playerCommand.substring(3)));
									}
								}
							}
